...
interface NewtonRaphsonProps {
  question: NewtonRaphsonInput[]
}

export default function NewtonRaphson({ question }: NewtonRaphsonProps) {
  const [form, setForm] = useState<NewtonRaphsonInput>(initialForm);
  const [result, setResult] = useState<NewtonRaphsonResult>();
  ...
  const cal = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/root/newton-raphson`, {
        method: "POST",
        headers: {
  ...
  const setSolution = (dataForm: NewtonRaphsonInput) => {
    setForm(dataForm);
    setOpenDialog(false);
  }
          ...
          {result?.ans && (
            <>
              <TabsContent value="solutions" className="flex flex-col gap-8">
                <DisplayResult>
                  <InlineMath math={`f'(x) = ${result.ans.fPrime}`} />
                </DisplayResult>
                <DisplayResult>
                ...
                {result.ans.iterations.map((data, index) => (
                  data.iter <= 4 && (
                    <DisplayResult key={index}>
                      ...
                      <InlineMath math={`x_{${data.iter-1}} = ${data.xOld}`} />
                      <InlineMath
                        math={`
                            x_{${data.iter}} = x_{${data.iter-1}} - \\frac{f(x_{${data.iter-1}})}{f'(x_{${data.iter-1}})} 
                            = ${data.xOld} - \\frac{${data.fx}}{${data.fPrimeX}} 
                            = ${data.xNew}
                          `}
                      />
                      <InlineMath
                        math={`
                          ...
                          = \\left| \\frac{${data.xNew} - ${data.xOld}}{${data.xNew}} \\right| * 100\\%
                          = ${data.tolerance}\\%
                        `}
                      />
                    </DisplayResult>
                  ...
                  <TableBody>
                    {result.ans.iterations.map((data, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell><InlineMath math={`${data.xOld}`}/></TableCell>
                        <TableCell><InlineMath math={`${data.xNew}`}/></TableCell>
