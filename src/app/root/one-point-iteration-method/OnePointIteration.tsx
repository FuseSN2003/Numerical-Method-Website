...
interface OnePointProps {
  question: OnePointInput[]
}

export default function OnePointIteration({ question }: OnePointProps) {
  const [form, setForm] = useState<OnePointInput>(initialForm);
  const [result, setResult] = useState<OnePointResult>();
  ...
  const cal = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/root/one-point`, {
        method: "POST",
        headers: {
  ...
  const setSolution = (dataForm: OnePointInput) => {
    setForm(dataForm);
    setOpenDialog(false);
  }
  ...
  return (
    <>
      ...
      <OnePointGraph
        ...
        ansPoint={result?.ans && [{x: result.ans.xOld, y: result.ans.x}]}
                ...
                {result.ans.iterations.map((data, index) => (
                  data.iter <= 4 && (
                    <DisplayResult key={index}>
                      ...
                      <InlineMath math={`x_{${data.iter}} = f(x_{${data.iter-1}}) = ${data.x}`} />
                      <InlineMath
                        math={`
