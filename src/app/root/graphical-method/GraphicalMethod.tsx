...
export default function GraphicalMethod({ question }: GraphicalMethodProps) {
  ...
  return (
    <>
      <div className="w-full max-w-2xl flex flex-col gap-8 mx-auto bg-white rounded-lg shadow-md p-8 border">
        ...
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
          <DialogTrigger tabIndex={-1} asChild>
            <Button className="w-1/2 mx-auto" variant="ghost">Select Solution</Button>
          </DialogTrigger>
      ...
      <RootOfEquationGraph points={result?.ans?.points} ansPoint={result?.ans && [{x: result.ans.x, y: result.ans.y}]} loading={loading} />
