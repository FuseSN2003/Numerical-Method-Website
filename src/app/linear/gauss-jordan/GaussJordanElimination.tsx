...
import { GaussJordanResult } from "@/lib/solutions/linearAlgebraEquation/LinearAlgebraEquation";
import { formatMatrix } from "@/lib/utils";
import { useRouter } from "next/navigation";
import { useState } from "react";
import { BlockMath, InlineMath } from "react-katex";

interface GaussJordanEliminationProps {
  question: any
}

export default function GaussJordanElimination({ question }: GaussJordanEliminationProps) {
  const [result, setResult] = useState<GaussJordanResult>();
  ...
  const handleCalculate = async (form: any, matrixA: number[][], matrixB: number[]) => {
    try {
      setLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/linear/gauss-jordan`, {
        method: "POST",
        headers: {
